...
Unordered_map<Key, Val, Hesh_method>::Unordered_map():obj(800) {};


template<typename Key, typename Val, typename Hesh_method >
Unordered_map<Key, Val, Hesh_method>::Unordered_map(const Key& k, const Val& v):obj(800)
{
...
Val& Unordered_map<Key, Val, Hesh_method>::operator[](const Key& ki)
{
	...
	typename std::list<std::pair<Key,Val>>::iterator itr = obj[ind].begin();
	itr->first = ki;
	return itr->second;
}
 ...
 {
	 uint32_t ind = hesh_str(ob.first, obj.getSize());
 ...
 {
	 uint32_t ind = hesh_str(ob, obj.getSize());
	 try
	 {
		 for (auto it = obj[ind].begin(); it != obj[ind].end(); ++it)
		{
